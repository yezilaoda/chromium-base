...
const char* const log_severity_names[] = {"", "][WARNING", "][ERROR",
                                          "][FATAL"};
    ...
    /*if (log_assert_handler_stack.IsCreated() &&
            ...
            if (log_assert_handler) {
                    log_assert_handler.Run(
                            file_, line_,
                            base::StringPiece(str_newline.c_str() +
    message_start_, stack_start - message_start_),
                            base::StringPiece(str_newline.c_str() +
    stack_start));
            }
    }
...
void LogMessage::Init(const char* file, int line) {
  ...
  if (g_log_timestamp) {
    ...
    stream_ << std::setfill('0') << std::setw(2) << local_time.wYear << '-'
            << std::setw(2) << local_time.wMonth << '-' << std::setw(2)
            << local_time.wDay << ' ' << std::setw(2) << local_time.wHour << ':'
            << std::setw(2) << local_time.wMinute << ':' << std::setw(2)
            << local_time.wSecond << '.' << std::setw(3)
